...
def init_db():
    ...
    c = conn.cursor()
    # Updated: Added 'full_name' column
...
def get_user(user_id, first_name="Unknown"):
    ...
    if not user:
        ...
        c.execute("INSERT INTO users (user_id, credits, role, generated_count, full_name) VALUES (?, ?, ?, 0, ?)",
                  (user_id, 0, 'Free', first_name))
        conn.commit()
        user = (user_id, 0, 'Free', 0, first_name)
    else:
        # Update name if changed (Optional but good)
...
async def generate_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db_user = get_user(user_id)  # Name update not needed here
    COST = 100
    ...
    conn = sqlite3.connect('minato_bot.db')
    c = conn.cursor()
    c.execute("SELECT id, email, password FROM accounts WHERE is_used=0 ORDER BY RANDOM() LIMIT 1")
    account = c.fetchone()

    if account:
        c.execute("UPDATE users SET credits = credits - ?, generated_count = generated_count + 1 WHERE user_id=?",
                  (COST, user_id))
        c.execute("UPDATE accounts SET is_used=1 WHERE id=?", (account[0],))
        conn.commit()
        ...
        keyboard = [[InlineKeyboardButton("✅ Working", callback_data='fb_working'),
                     InlineKeyboardButton("❌ Not Working", callback_data='fb_not_working')]]
...
async def feedback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'fb_working':
        ...
        await query.message.edit_text("❌ **REPORT MODE**\nPlease send a screenshot of the error now.",
                                      parse_mode='Markdown')
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mention Link for Admin to click
    ...
    if state == 'report':
        ...
        keyboard = [[InlineKeyboardButton(f"♻️ Refund 100 Cr", callback_data=f"refund_{user.id}")],
    ...
    try:
        await context.bot.send_photo(chat_id=ADMIN_LOG_ID, photo=photo, caption=caption,
...
async def admin_log_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("refund_"):
        ...
        try:
            await context.bot.send_message(target_id, "✅ 100 Credits Refunded!")
        except:
            pass
        ...
        try:
            await context.bot.send_message(target_id, f"✅ **Payment Received!**\nPackage: {plan}\nCredits: +{amount}")
        except:
            pass
        try:
            await context.bot.send_message(PUBLIC_LOG_ID,
                                           f"⚡ **PREMIUM PURCHASED!**\n👤 User: `{target_id}`\n💎 Plan: `{plan}`",
                                           parse_mode='Markdown')
        except:
            pass
...
# 6. ADMIN COMMANDS (ACTIVE USERS WITH CLICKABLE NAME)
async def active_users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return

    conn = sqlite3.connect('minato_bot.db')
    c = conn.cursor()
    # Fetch Name, ID, Credits, Generated Count
    ...
    for i, u in enumerate(users, 1):
        ...
        # This creates a clickable link to the user
...
async def admin_get_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    acc = c.fetchone()
    if acc:
        c.execute("UPDATE accounts SET is_used=1 WHERE id=?", (acc[0],))
        conn.commit()
    ...
    try:
        ...
        with open("stock.txt", 'r', encoding='utf-8', errors='ignore') as f:
            for email, password in pattern.findall(f.read()):
                c.execute("INSERT INTO accounts (email, password, is_used) VALUES (?, ?, 0)", (email, password))
                count += 1
        conn.commit();
        conn.close();
        os.remove("stock.txt")
...
async def gen_code_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sqlite3.connect('minato_bot.db').cursor().execute("INSERT INTO codes VALUES (?,?,?,0)",
                                                          (code, amt, role)).connection.commit()
...
async def add_credit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sqlite3.connect('minato_bot.db').cursor().execute("UPDATE users SET credits=credits+? WHERE user_id=?",
                                                          (amt, tid)).connection.commit()
...
async def redeem_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if res:
            ...
            try:
                await context.bot.send_message(PUBLIC_LOG_ID, f"⚡ **REDEEMED!**\n👤 User: `{uid}`\n💎 Role: `{res[2]}`",
                                               parse_mode='Markdown')
            except:
                pass
        else:
...
async def btn_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith(('pay_', 'refund_', 'reject_')):
        await admin_log_actions(update, context)
    elif q.data == 'gen_acc':
        await generate_account(update, context)
    elif q.data == 'profile':
        await start(update, context)
    elif q.data == 'deposit_info':
        await deposit_info(update, context)
    elif q.data in ['fb_working', 'fb_not_working']:
        await feedback_handler(update, context)
    elif q.data == 'redeem_btn':
        await q.answer(); await q.message.reply_text("Type `/redeem CODE`")
